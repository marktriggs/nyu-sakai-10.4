...
function setupSortableEdit(msgs){
	...
	$("table.editTable tbody").sortable({
		...
		stop: function( event, ui ) {
			...
			if(moved != 0){
				//update the position:
				var itemId = $(ui.item).find(".syllabusItem").text();
				$("#moveItem").val(itemId);
				$("#moveSteps").val(moved);
				$("a.move").click();
				//postAjax($(ui.item).find(".syllabusItem").text(), {"move": moved}, msgs);
			}
		}
	});
