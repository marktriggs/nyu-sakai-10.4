...
public class SyllabusTool
{
      ...
      throws org.sakaiproject.exception.PermissionException
  {
    ...
    try
    {
      ...
      boolean noEntries = entries != null && entries.size() == deletedCount;
      entries.clear();
      entry = null;
      if(noEntries){
    	  return "main";
  ...
  public String processEditBulkCancel()
  {
	...
	private String processEditBulk(boolean post) throws PermissionException{
		try{
			...
			boolean addByItems = "1".equals(bulkEntry.getAddByItems());
			int bulkItems = -1;
			if(bulkEntry != null){
				//check title:
				if(bulkEntry.getTitle() == null || bulkEntry.getTitle().trim().isEmpty())
				{
					...
					try{
						if(bulkEntry == null || bulkEntry.getBulkItems() == null || "".equals(bulkEntry.getBulkItems().trim())){
							//default to 1 item							
							bulkItems = 1;
						}else{
				...
				if(alertMessage != null){
					...
					if(addByDate){
						...
						//add by bulk items
						SyllabusData data = null;
						for(int i = 1; i <= bulkItems; i++){
							String title = bulkEntry.getTitle();
							if(addByItems){
								title += " - " + i;
							}
							data = syllabusManager.createSyllabusDataObject(title,  
									new Integer(initPosition), null, "no", status, "none", null, null, false, null, null, syllabusItem);
							syllabusManager.addSyllabusToSyllabusItem(syllabusItem, data, false);
							initPosition++;
						}
						if(bulkItems == 1 && data != null){
							//we only created one item, just jump straight to it
							DecoratedSyllabusEntry daEntry = new DecoratedSyllabusEntry(data);
							return daEntry.processListRead();
						}
					}
					if(mainEdit){
						return "main_edit";
    ...
    try
    {
      ...
      else
      {
        ...
        if (entry.justCreated == false)
        {
          ...
          //make the main page display this item automatically when loading the page:
          ToolSession currentToolSession = SessionManager.getCurrentToolSession();
          currentToolSession.setAttribute(SESSION_ATTACHMENT_DATA_ID, getEntry().getEntry().getSyllabusId().toString());
  ...
  public String processMove(){
  	String moveItem = (String) FacesContext.getCurrentInstance().
  			getExternalContext().getRequestParameterMap().get("moveItem");
  	String moveSteps = (String) FacesContext.getCurrentInstance().
  			getExternalContext().getRequestParameterMap().get("moveSteps");
  	int paramSteps = Integer.parseInt(moveSteps);
  	long paramId = Long.parseLong(moveItem);
  	if(entries != null){
  		Iterator iter = entries.iterator();
  		while (iter.hasNext())
  		{
  			DecoratedSyllabusEntry en = (DecoratedSyllabusEntry) iter.next();
  			if(paramId == en.getEntry().getSyllabusId()){
  				if(paramSteps > 0){
  					for(int i = 0; i < paramSteps; i++){
  						upOnePlace(en.getEntry());
  					}
  				}else{
  					for(int i = paramSteps; i < 0; i++){
  						downOnePlace(en.getEntry());
  					}
  				}
  			}
  		}
...
//    entry = null;
  ...
  public String processRedirectMain() throws PermissionException{
	  mainEdit = false;
	  return processRedirect();
  }
  
  public String processRedirectMainEdit() throws PermissionException{
	  mainEdit = true;
	  return processRedirect();
  }
...
//for test        entry.getEntry().getAsset(), null, null, null);
  ...
  public class BulkSyllabusEntry{
	  ...
	  private String addByOne = "1";
	  
	public String getAddByOne() {
		return addByOne;
	}
	public void setAddByOne(String addByOne) {
		this.addByOne = addByOne;
	}
...
public String getOpenDataId() {
	ToolSession session = SessionManager.getCurrentToolSession();
	if(session.getAttribute(SESSION_ATTACHMENT_DATA_ID) != null){
		String id = (String) session.getAttribute(SESSION_ATTACHMENT_DATA_ID);
		session.removeAttribute(SESSION_ATTACHMENT_DATA_ID);
		return id;
	}else{
		return "";
